...
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--runtime",required=True);a=ap.parse_args()
    ...
    new="""        self._load_weights()
        if __import__("os").getenv("FENIX_TRACE","0").lower() in {"1","true","yes"}:
            from fenix_trace_runtime import emit, next_id
...
"""
    ...
    if _fenix_trace:
        from fenix_trace_runtime import next_id
...
""")
    ...
    old="""            _update_lru_expert_map_kernel[(1,)](
                ...
                cache.miss_slots,
                num_ids=num_ids,
                global_num_experts=layer.expert_map.numel(),
                capacity=capacity,
