#!/usr/bin/env python3
"""Launch H1/H2-equivalent trace server for resident/mmap PLE placement control."""
...
def build_control_command(
    *,
    ...
    ple_storage_mode: str,
    ple_bank_manifest: Path | None,
...
def main() -> int:
    ...
    parser.add_argument("--ple-storage-mode", choices=launch_vllm.PLE_STORAGE_MODES, required=True)
    parser.add_argument("--ple-bank-manifest", type=Path)
    ...
    bank = args.ple_bank_manifest.resolve() if args.ple_bank_manifest is not None else None
    ...
    if not (runtime_directory / ".git").is_dir():
        ...
        return 2
    if args.ple_storage_mode == "mmap" and (bank is None or not bank.is_file()):
        print("mmap placement requires an existing --ple-bank-manifest")
        return 2
    if args.ple_storage_mode == "resident" and bank is not None:
        print("resident placement does not accept --ple-bank-manifest")
        return 2

    environment, command = build_control_command(
        model_directory=model_directory,
        runtime_directory=runtime_directory,
        trace_directory=trace_directory,
        gpu_ids=args.gpus,
